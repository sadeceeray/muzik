...
module.exports = {
  ...
  cooldown: 5,
  aliases: ["pl"],
  description: "Youtube den Playlist oynatır.",
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`**Kullanımı:** ${message.client.prefix}playlist <YouTube Oynatma listesi URL | Playlist ismi>`)
        .catch(console.error);
    if (!channel) return message.reply("Öncelikle bir ses kanalına katılmak gerekli!").catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.reply("Ses kanalına bağlanılamıyor, izinlerimi kontrol et!");
    ...
    if (urlValid) {
      ...
      if (args[0].includes("/sets/")) {
        message.channel.send("⌛ Oynatma listesi hazırlanılıyor...");
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        await channel.leave();
        return message.channel.send(`Kanala Katılamadım: ${error.message}`).catch(console.error);
      }
    }
  }
};
